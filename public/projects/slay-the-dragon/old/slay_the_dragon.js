...
function sound(soundID,delay){
	if(!mute){
		setTimeout(function(){document.getElementById(soundID + "_sound").play(); }, delay)
	}
}
...
setTimeout(escapeBattle,2500);});}else{
    setTimeout(function(){yourTurn=true;},700);}
};
  ...
  {rotation: 110},
  {
    ...
    step: function(now, fx) {
      ...
      if(items == 3 || items == 4){
    	  ...
    	  sound("wood_blade",800)
      }
    }
    
  }
);
$('#monster').stop().animate({left:'200px'}, 1000);
...
monsterhit(1200);
$('#monster').delay(1200).animate({left:'550px'}, 1000);
...
$('#playerCharacter').delay(1200).animate({left:'0px'}, 1000); //-=350px
...
value: pMana/pMaxMana*100
        ...
        //animaatio
        document.getElementById("heal_sound").pause();
        document.getElementById("heal_sound").currentTime = 0;
        sound("heal",0);
        $('#playerDrain').fadeIn(1000,function(){
...
switch(enemyType){
    ...
    case 3:
        ...
        $('#monsterArm').css('background','url(img/skeleton_arm.png)');
        $('#dragonMouth').css('background','none');
    break;
    case 4:
        monsterName='Dragon';
...
$('#credits').click(function(){
    $('#name').fadeOut(0);
    insertHowToPlay("<table style='font-size:22px'><tr><td>Programming:</td><td>Voya</td></tr><tr><td>Artwork:</td><td>Voya</td><tr><td>Sound effects:</td><td>Freesoung.org (Public domain)</td></tr></table>");});
...
$('#namebutton').click(function(){
        ...
        $('#username').html(username);
        $('#saveMessage').fadeIn(500).delay(3000).fadeOut(500);
});
});
