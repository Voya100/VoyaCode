...
export class Pawn extends Piece{
  ...
  checkVerticalTile(distance: number){
    ...
    const tile: Tile = this.tiles[y][this.x()];
  ...
  checkDiagonalTile(xDir: number){
    ...
    const	tile = this.tiles[y+this.yDir][x+xDir];
    const horizontalTile = this.tiles[y][x+xDir];
  ...
  tryToDoEnPassant(x: number, y: number){
    const tile = this.player.game.board[y][x];
